...
class Address_View(APIView):
    def get(self, request, rid=None, format=None):
        try:
            if rid is not None:
                user=Address_Model.objects.get(rid=rid)
            else:
                user=Address_Model.objects.all()
        except Address_Model.DoesNotExist:
            return Response("Data not found : id may wrong ",status=status.HTTP_404_NOT_FOUND)
        if rid is not None:
    ...
    def post(self,request,format=None):
        try:
            ...
            if alternate_number==None or alternate_number=='':
                python_data={
                'rid':rid,
    ...
    def put(self,request,rid=None,format=None):
        try:
            user=Address_Model.objects.get(rid=rid)
        ...
        return Response(serializer.errors,status=status.HTTP_400_BAD_REQUEST)
    def delete(self,request,rid=None,format=None):
        try:
            if rid==None:
